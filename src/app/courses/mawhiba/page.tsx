...
export default function MawhibaPage() {
  ...
  useEffect(() => {
    ...
    const loadImage = async () => {
      console.log(`[DebugImage] Page: MawhibaPage, ID: ${IMAGE_DETAIL.id}. Initiating image load. Hint: "${IMAGE_DETAIL.hint}", Original: ${IMAGE_DETAIL.originalSrc}`);
      try {
        const result = await generateImageFromHint({ hint: IMAGE_DETAIL.hint });
        if (isMounted) {
          if (result.imageDataUri === IMAGE_GENERATION_FAILED_FALLBACK) {
            console.warn(`[DebugImage] Page: MawhibaPage, ID: ${IMAGE_DETAIL.id}. AI FAILED or FALLBACK. Attempting to set placeholder: ${IMAGE_DETAIL.originalSrc}`);
            setHeaderImageUrl(IMAGE_DETAIL.originalSrc);
          } else {
             console.log(`[DebugImage] Page: MawhibaPage, ID: ${IMAGE_DETAIL.id}. AI SUCCEEDED. Attempting to set AI image (first 100 chars): ${result.imageDataUri.substring(0,100)}...`);
            setHeaderImageUrl(result.imageDataUri);
          }
        }
      } catch (error) {
        console.error(`[DebugImage] Page: MawhibaPage, ID: ${IMAGE_DETAIL.id}. EXCEPTION caught for hint "${IMAGE_DETAIL.hint}":`, error);
        if (isMounted) {
          console.warn(`[DebugImage] Page: MawhibaPage, ID: ${IMAGE_DETAIL.id}. EXCEPTION. Attempting to set placeholder: ${IMAGE_DETAIL.originalSrc}`);
          setHeaderImageUrl(IMAGE_DETAIL.originalSrc);
        }
      }
    };
